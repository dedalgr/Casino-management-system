...
class PostgreSQL(SQLdb):
    ...
    def connect(self):
        try:
            self.conn = psycopg2.connect(dbname=self.dbname, user=self.user, host=self.host, password=self.passwd,
                                         port=self.port)
        except Exception as e:
            raise NoSQLDBConnection(e)
        self.db = self.conn.cursor()
    ...
    def _backup(self, backup_path, table_names=None, name=None):
        ...
        if name == None:
            ...
            filename = now.strftime(name + '_' + "%d-%m-%Y_%H-%M-%S") + '.backup'
...
if __name__ == '__main__':
    a = SQLite('/home/dedal/log.db')
    a.connect()
    print(a.get("select value from keys where key='2022-07-26'"))
