...
def Read_RTC(bus=conf.RTC_Bus, zone=conf.RTC_TIME_ZONE):
    ...
    if conf.RTC_BUG_FIX == True:
        ...
        try:
            ...
            exception.log.stderr_logger.critical('%s', 'found rtc bug use  -b 1 option')
...
def Write_RTC(bus=conf.RTC_Bus, init_write=False):
    "Write system clock to MOD-RTC"
    bus = smbus.SMBus(bus)
    a = time.time()
    if init_write == True:
        DB.set_key_to('INIT', 'init_time', a)
    a = time.gmtime(a)
    buf = []
