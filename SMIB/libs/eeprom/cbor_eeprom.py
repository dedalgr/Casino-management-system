...
from .device_types import EEPROM_TYPES, FROM_BLOCK
from .util import check_device_exists
...
class CBOR_EEPROM(EEPROM):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._chunk_size = int(self._size / 128 - FROM_BLOCK)
        self._data = None 
    ...
    def read_file(self):
        if self._data is None:
            ...
            if self.read(1) == b'\xff':
                ...
                return self._data
            rd_data = b''
            for x in range(FROM_BLOCK, self._size, self._chunk_size):
    ...
    def erase_file(self, del_all=False):
        # Invalidate file
        if del_all is False:
            self.write(b'\xff' * self._chunk_size)
        else:
            self.write(b'\xff' * int(self._size / 128 ), addr=0)
        self._data = {}
