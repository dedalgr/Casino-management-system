...
class RFID_RC522():
    def __init__(self, port, app_name, scan_time=500):
        self.port = port
        self.app_name = app_name
        self.scan_time = scan_time*10
        # self.ranges = int(scan_time*0.01)

    def get_id(self):
        data = 'NONE'
        for i in range(3):
            data = os.popen('%s -d %s --id -n 5 -y %s' % (self.app_name, self.port, self.scan_time)).read()
            data = data[:-1]
            data = data.upper()
            if data != 'NONE':
                try:
...
if __name__ == '__main__':
    cart = RFID('/dev/rfid', timeout=1 )
    cart.open()
    print (cart.scan_time(200))
    count = 1
    while True:
