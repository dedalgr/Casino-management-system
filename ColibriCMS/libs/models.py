...
if __package__ == 'libs' and '/home/dedal/Colibri/ColibriCMS/2_1/gui_lib' not in sys.path:
    # import conf
    from . import conf
    from . import rtc
    from . import udp
    tz = udp.send('get_tz', ip=conf.SERVER)
    try:
        if tz != None:
            TZ = rtc.TZFormat(tz)
        else:
            tz = 'Europe/Sofia'
            TZ = rtc.TZFormat('Europe/Sofia')
    ...
    engine = create_engine('postgresql://%s:%s@%s:%s/%s' %
                           (conf.DB_USER, conf.DB_PASS, conf.SERVER, conf.DB_PORT, conf.DB_NAME),
                           echo=conf.DB_DEBUG,
                           echo_pool=False,
    ...
    try:
        ...
        import rtc
        import udp
    ip_work = input('SET IP ADRESS: ')
    tz = 'Europe/Sofia'
    TZ = rtc.TZFormat('Europe/Sofia')
    if ip_work == '':
        ip_work = '127.0.0.1'
    engine = create_engine('postgresql://mistralcms:use_system10@%s:5432/mistralcms' % (ip_work), # Пишеш IP на казинино
                           echo=True,
                           echo_pool=False,
                           pool_reset_on_return=True,
                           connect_args={'connect_timeout': 30},
...
class KasaTransfer(Base):
    ...
    pub_time = Column(DateTime, nullable=False, default=TZ.now)
...
class CashOutPrinted(Base):
    ...
    id = Column(Integer, primary_key=True)
    mony = Column(Float, nullable=False, unique=False, default=0)
    # prefix = Column(Integer, unique=True, nullable=False, default=0)
    cust_id = Column(Integer, ForeignKey(CustUser.id), nullable=True, unique=False)
    cust = relationship(CustUser, foreign_keys=[cust_id], backref="cust_print_rko")
    # name = Column(Text, nullable=True, unique=False)
    # egn = Column(String(10), unique=False, nullable=True)
    # adress = Column(Text, nullable=True, unique=False)
    # sity = Column(String(24), nullable=True, unique=False)
    pub_time = Column(DateTime, nullable=False, default=TZ.now)
    pub_user_id = Column(Integer, ForeignKey(User.id), nullable=False, unique=False)
...
class CartPrise(Base):
    ...
    mony = Column(Float, nullable=False, unique=False)
    player_id = Column(Integer, ForeignKey(CustUser.id), nullable=True)
    player = relationship(CustUser, foreign_keys=player_id, backref="player_cart_price")
    chk = Column(Boolean, nullable=False, unique=False, default=False)
...
def copy_day_report(dates, from_nom, start_nom=None):
    db = DBCtrl()
    data = db.get_one_where(DayReport, id=from_nom)  # Номера на отчета който искаш да копираш
    if start_nom != None:
        nom = start_nom
    else:
        nom = data.doc_nom
        nom += 1
    vat = json.loads(data.doc_data)
    for i in dates:
        # print type(vat['nom'])

        vat['nom'] = nom
        # vat['total_out'] = '0.00'
        # vat['sum_all_total'] = '0.00'
        # vat['total_in'] = '0.00'
        vat['doc_date'] = i
        # vat['total'] = 0
        # for i in vat['row']:
        #     # print vat['row'][i]['total_out'], vat['row'][i]['total_in'], vat['row'][i]['total']
        #     vat['row'][i]['total_out'] = '0.00'
        #     vat['row'][i]['total_in'] = '0.00'
        #     vat['row'][i]['total'] = '0.00'
        new_data = db.make_obj(DayReport)
        new_data.user_id = data.user_id
        new_data.day_report = True
        new_data.doc_nom = nom
        new_data.doc_data = json.dumps(vat)
        new_dates = datetime.datetime.strptime(i, '%d-%m-%Y')
        new_dates = datetime.datetime.strftime(new_dates, '%Y-%m-%d')
        new_data.pub_time = new_dates + ' 08:00:00'
        db.add_object_to_session(new_data)
        nom += 1
