...
class ConfFile():
    ...
    def __init__(self, confName, make_new=False):
        ...
        # Създава парсер
        if make_new is True:
            open(confName, 'a', encoding='utf-8').close()

        self.parser = ConfigParser(allow_no_value=True)
        try:
            open(confName, 'r', encoding='utf-8')  # @UnusedVariable
        except IOError as e:
    ...
    def update_option(self, section, **option):
        ...
        Връща:
            True
        '''
        self.parser.read(self.file_name, encoding='utf-8')

        for i in list(option.keys()):
            var = self.parser.has_option(section, i)
            if var is True:
                # if type(option[i]) == str:
                #     option[i] = ''.join(option[i]).encode('utf-8')
                self.parser.set(section, i, str(option[i]))

            else:
                raise ConfError('Option %s not exist!' % (i))
        myfile = open(self.file_name, 'w', encoding='utf-8')
